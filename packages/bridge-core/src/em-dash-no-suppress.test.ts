...
test("em-dash does NOT trigger bot-tell suppression", () => {
  ...
  const v = detectBotTells("he's married to Manasa — that's you 😄");
...
test("naturalize rewrites the em-dash and still produces a non-empty reply", () => {
  const pieces = naturalize("he's married to Manasa — that's you 😄", {
  ...
  assert.match(joined, /married to manasa/i);
});
