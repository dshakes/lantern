...
test("parses a conference [CALL] marker", () => {
  const r = extractActionMarkers("sure, setting that up.\n[CALL:Manasa|conference|owner wants to talk]");
  assert.equal(r.calls.length, 1);
  assert.equal(r.calls[0].target, "Manasa");
...
test("no marker → no calls, text untouched", () => {
  const r = extractActionMarkers("i'll reach out to Manasa when she's free");
  assert.equal(r.calls.length, 0);
  assert.equal(r.cleanedText, "i'll reach out to Manasa when she's free");
});
