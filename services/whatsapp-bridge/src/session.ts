...
export class WhatsAppSession {
  ...
  private async applyDisclosureDeny(target: string, deny: boolean): Promise<boolean> {
  ...
  private async maybeRouteIntent(jid: string, text: string): Promise<boolean> {
    try {
      ...
      if (handler === "disclosure_deny" && decision?.kind === "disclosure_deny") {
        return await this.applyDisclosureDeny(decision.target, decision.deny);
      }
  ...
  private async runNewsProactiveTick(): Promise<void> {
    try {
      ...
      // owner-local). Persisted to a sibling file so a same-day restart doesn't
      // re-send it.
      const digestHour = parseInt(process.env.LANTERN_NEWS_DIGEST_HOUR ?? "9", 10);
      const now = new Date();
      const ymd = `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}`;
      const digestDayPath = this.newsPushedFile.replace(/[^/]+$/, "news-digest-day.txt");
