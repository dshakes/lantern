...
export class MediaHandler {
  ...
  private async transcribeVoice(buf: Buffer, mime?: string): Promise<MediaAnnotation> {
    ...
    if (!apiKey) {
      return {
        ok: false, kind: "voice", mime,
        syntheticText: "[voice note — transcription unavailable. Set OPENAI_API_KEY to enable Whisper.]",
      };
    }
    try {
      const form = new FormData();
      // Whisper accepts ogg/m4a/mp3/wav/caf etc. — pass the source
      // type unchanged when present so Whisper picks the right decoder.
      const filename = mime?.includes("caf") ? "voice.caf" : mime?.includes("mp4") ? "voice.m4a" : "voice.audio";
      ...
      if (!res.ok) {
        ...
        return { ok: false, kind: "voice", mime, syntheticText: `[voice note — Whisper returned ${res.status}]` };
      }
      const data = (await res.json()) as { text?: string };
      const transcript = (data.text || "").trim();
      if (!transcript) return { ok: false, kind: "voice", mime, syntheticText: "[voice note — empty transcription]" };
