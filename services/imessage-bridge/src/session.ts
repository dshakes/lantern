...
// One channel's recent thread for a person — the unit exchanged between bridges
// over loopback so "what did X say" answers identically on both.
...
export class IMessageSession {
  ...
  // handle; one-shot; expires with OFFER_TTL_MS.
  ...
  // (todos + calendar + owner heads-up). Default ON (owner asked for it).
  ...
  // ── SKILL FORGE (owner-taught recurring skills) ────────────────────
  // "new skill: ..." → LLM drafts a spec → owner approves → fires on the
  // proactive tick (killswitch/mute/quiet-hours gated like every loop).
  ...
  private ensureSkillState(): SkillForgeState {
    ...
    try {
      if (existsSync(this.skillPath)) {
        this.skillState = { ...defaultSkillState(), ...(JSON.parse(readFileSync(this.skillPath, "utf8")) as SkillForgeState) };
      }
    } catch (err) {
  ...
  /** Owner self-chat hook (both routing paths). Returns true when handled. */
  private maybeHandleSkillForge(jid: string, text: string): boolean {
    const t = text.trim();
    const cmd = matchSkillCommand(t);
    if (cmd === "list") {
      void this.send(jid, formatSkillList(this.ensureSkillState().skills)).catch(() => {});
      return true;
    }
    if (cmd && typeof cmd === "object") {
      ...
      if (idx < 0 || idx >= st.skills.length) {
        void this.send(jid, `🛠 no skill ${cmd.drop} — "skills" to see the list.`).catch(() => {});
      } else {
        const [gone] = st.skills.splice(idx, 1);
        this.persistSkillState();
        void this.send(jid, `🛠 dropped ${gone.name}.`).catch(() => {});
      }
      return true;
    }
    // Approve / discard a pending proposal (10-min TTL).
    if (this.pendingSkillProposal && Date.now() - this.pendingSkillProposal.at < 10 * 60_000) {
      if (/^(approve skill|approve|activate)$/i.test(t)) {
        ...
        this.persistSkillState();
        void this.send(jid, `✅ skill live — ${spec.description} (${formatSchedule(spec.schedule)}). "skills" to manage.`).catch(() => {});
        return true;
      }
      if (/^(drop|discard|no|skip|cancel)$/i.test(t)) {
        this.pendingSkillProposal = null;
        void this.send(jid, `👍 skipped`).catch(() => {});
        return true;
      }
    }
    ...
    void (async () => {
      try {
        ...
        if (!spec) {
          await this.send(jid, `🛠 couldn't shape that into a skill — try rephrasing what and when.`);
        } else if ("error" in spec) {
          await this.send(jid, `🛠 can't do that one: ${spec.error}`);
        } else {
          this.pendingSkillProposal = { spec, at: Date.now() };
          await this.send(jid, formatSkillProposal(spec));
        }
      } catch (err) {
        this.logger.warn({ err }, "skill spec draft failed");
        await this.send(jid, `🛠 skill drafting hit an error — try again.`).catch(() => {});
      }
    })();
    return true;
  }
  ...
  private maybeRunSkills(target: string, now: number): void {
    const st = this.ensureSkillState();
    if (st.skills.length === 0) return;
    const tz = process.env.LANTERN_OWNER_TIMEZONE;
    const due = dueSkills(st, new Date(now), tz, 50);
    for (const s of due) {
      // Mark BEFORE executing: a crash mid-run must not double-fire (missing
      // one delivery beats a duplicate).
      ...
      void (async () => {
        try {
          ...
          if (out?.trim()) await this.send(target, `🛠 ${s.name}: ${out.trim()}`);
        } catch (err) {
   ...
   *  staged plan; returns true when it owns the message. The LLM decomposes;
   *  the owner confirms ONCE; each step runs through the existing executors.
  ...
  private maybeHandlePlan(jid: string, text: string): boolean {
    ...
    if (this.pendingPlan && Date.now() - this.pendingPlan.at < PLAN_TTL_MS) {
      ...
      if (reply && "cancel" in reply) {
        ...
        void this.send(jid, "🗂 scrapped.").catch(() => {});
        return true;
      }
      if (reply && "drop" in reply) {
        ...
        if (next.steps.length === 0) {
          this.pendingPlan = null;
          void this.send(jid, "🗂 nothing left — scrapped.").catch(() => {});
        } else {
          this.pendingPlan = { plan: next, at: Date.now() };
          void this.send(jid, formatPlanPreview(next)).catch(() => {});
        }
        return true;
      }
  ...
  private async runPlanDecompose(jid: string, request: string): Promise<void> {
    ...
    if (!plan) {
      // Silent fall-through would strand the owner; nudge once.
      await this.send(jid, "🗂 couldn't turn that into a plan — try one action at a time.").catch(() => {});
      return;
    }
    this.pendingPlan = { plan, at: Date.now() };
    await this.send(jid, formatPlanPreview(plan)).catch(() => {});
  }

  /** Execute a confirmed plan through existing executors, best-effort per step. */
  private async executePlan(jid: string, plan: ActionPlan): Promise<void> {
    const done: string[] = [];
    const skipped: string[] = [];
    let staged = false; // a message draft parked in the B5 rail this run
    for (const s of plan.steps) {
      ...
      try {
        if (s.kind === "calendar") {
          ...
          else skipped.push(`${s.title} (${res.reason ?? "failed"})`);
        } else if (s.kind === "note") {
          ...
          const drafted = (await this.agent
            .respondTo(`plan::msg::${target.phone}`,
          ...
          if (!staged) {
            // Park in the single-slot drafts rail; owner says "send" to fire.
            this.pendingDraftEdits.set(this.ownerSelfChatTarget(), {
              target: target.phone, targetLabel: target.name ?? s.contact,
              draft: drafted, inbound: "", issuedAt: Date.now(),
            });
            staged = true;
            done.push(`drafted to ${target.name ?? s.contact}`);
          } else {
            // Additional message steps can't share the slot — surface the text.
            done.push(`draft to ${target.name ?? s.contact}: “${drafted}”`);
          }
        }
      } catch (err) {
    ...
    const tail = staged ? "\nreply 'send' to fire the draft." : "";
    await this.send(jid, `🗂 ${parts.join(" · ") || "nothing to do"}${tail}`).catch(() => {});
  }
      ...
      if (cmd === "brief") {
        ...
        try {
          ...
          agentStats = mergeBridgeLoopStatus(agentStats, [
            { name: "commute-copilot", enabled: IMessageSession.COMMUTE_ENABLED, firedToday: this.commuteDriveFired },
            { name: "energy-guardian", enabled: IMessageSession.ENERGY_ENABLED, firedToday: this.energyNudgedDate === ymd },
            { name: "health-coach", enabled: IMessageSession.HEALTH_ENABLED, firedToday: this.healthNudgedDate === ymd },
            { name: "focus-guardian", enabled: IMessageSession.FOCUS_ENABLED, note: this.focusWasActive ? "in focus" : undefined },
          ]);
        } catch { /* best-effort */ }
        ...
        // LLM-curated /v1/news/ask (understands authors/companies/topics/major),
        // grouped by company with dates. Falls back to the structured feed on error.
        const nq = cmd.news;
        const q = nq.source
          ? nq.source
          : nq.category
        ...
        } catch { /* fall through to structured feed */ }
        if (!nv) {
   ...
   * real confirmation in the owner's voice, and DMs the owner a heads-up.
   * Returns true when it handled the message (caller skips the normal reply).
   * Best-effort; never throws — returns false so the normal reply runs.
   */
  private async runSpouseAgent(handle: string, text: string, displayName?: string): Promise<boolean> {
    try {
      ...
      for (const item of resp.items) {
        await this.commitments
          .create({
            ...
            idempotencyKey: `spouse:${norm(handle)}:${norm(item.title)}:${day}`,
  ...
  private async autoActLifeEvent(
    ...
    try {
      ...
      this.pendingBookingConfirm.set(owner, {
      ...
      await this.send(owner, `📅 book '${title}' — ${friendly}? reply yes to add`).catch(() => {});
    } catch (err) {
  ...
  private async bookPendingCalendar(
    ...
    if (!res.ok) {
      await this.send(owner, `couldn't add it — ${res.reason}`).catch(() => {});
      return;
    }
    ...
    await this.send(owner, log).catch(() => {});
    this.noteAutoAction(log);
    this.armAutoActUndo(owner, p.lifeEventKind, p.idempotencyKey, { undoTarget: "calendar", undoTitle: p.title, undoStartIso: p.startIso }, log, p.rawText);
    // Emit auto_acted to Automations dashboard (best-effort, fire-and-forget).
    void (async () => {
      try {
        const { emitLifeEvent } = await import("@lantern/bridge-core/life-events-emit");
  ...
  // LOCAL single-channel peek: resolve the contact to their most-active iMessage
  // thread and return recent messages as DATA. PUBLIC so the WhatsApp bridge can
  // fetch it over loopback (cross-bridge parity — both bridges answer identically
  // across iMessage + WhatsApp + email).
  ...
  private isPhoneish(to: string): boolean {
    return !to.includes("@") && to.replace(/[^\d]/g, "").length >= 8;
  }

  /**
   * After an iMessage send, re-check delivery from chat.db ~30s later (the
   * "Not Delivered" error is computed async). If the just-sent iMessage failed
   * (service=iMessage, error!=0, undelivered), fall back to SMS once so the
   * message still gets there. Best-effort; unref'd so it never holds the process.
   */
  private scheduleDeliveryWatch(to: string, text: string): void {
    ...
    const t = setTimeout(() => {
      try {
        ...
        this.logger.debug({ err }, "delivery watch failed (no-op)");
      }
    }, delayMs);
    t.unref?.();
  }
  ...
  private async relayDocRequestToOwner(contactHandle: string, contactLabel: string, request: string, initiatedByOwner = false, targetChannel: "imessage" | "whatsapp" | "sms" = "imessage"): Promise<void> {
    ...
    if (hit) {
      ...
      const chLabel = targetChannel === "whatsapp" ? " via WhatsApp" : targetChannel === "sms" ? " via SMS" : "";
    ...
    if (pickList && pickList.length) {
      ...
      this.pendingDraftEdits.set(`${ownerThread}::${contactHandle}`, {
        ...
        kind: "doc-pick",
        candidates: pickList,
        channel: targetChannel, // carried into the doc-relay pending on pick (…pe spread) so a cross-bridge choice still relays
      });
  ...
  private async deliverFileToContact(
    ...
    // Rather than let a confirmed file silently vanish, make ONE best-effort
    // inline iMessage attempt — but ONLY for a phone with NO send history
    // (recent === null: unknown capability). We must NOT gamble on iMessage for
    // a handle with history proving it's SMS/RCS or a prior iMessage failure —
    // that's exactly chooseFileTransport's "never gamble on iMessage" rule, and
    // the AppleScript exit code would falsely report success on a dropped SMS.
    // The exit code can't fully confirm receipt even here, so the caller reports
  ...
  private async deliverPendingSend(
    ...
    if (pending.kind === "doc-relay" && pending.filePath) {
      // Cross-bridge: owner asked (from iMessage) to send this doc over WhatsApp.
      // The confirm happened here; the delivery relays to the WhatsApp bridge,
      // which attaches the SAME file (shared Mac) on its own wire and reports ok
      // back — so the ✅ ack stays delivery-backed, never a cross-channel lie.
      if (pending.channel === "whatsapp") {
        return this.relayDocToPeer(
          (process.env.LANTERN_WHATSAPP_BRIDGE_URL || "http://127.0.0.1:3100").replace(/\/$/, ""),
          pending.target,
          pending.filePath,
          pending.inbound || "file",
        );
      }
  ...
  private handleNewRow(row: IMessageRow): void {
    ...
    if (row.isFromMe) {
      ...
      if (this.maybeHandlePlan(row.handle || this.ownHandleGuess() || this.lastSelfHandle || "", text)) return;
  ...
  private async handleInbound(row: IMessageRow, unixMs: number, isGroup: boolean): Promise<void> {
    ...
    if (text && !isGroup && this.isOwnerChatRow(row)) {
      ...
      if (this.maybeHandlePlan(row.handle, text)) return;
    }
    ...
    if (this.personalDocsEnabled && !isGroup && this.isOwnerChatRow(row) && text) {
      ...
      // Must run BEFORE the trivial-chatter branch so "yes" fires the book.
      const pb = this.pendingBookingConfirm.get(row.handle);
      if (pb) {
        const outcome = resolvePendingBooking(pb, text, Date.now(), IMessageSession.OFFER_TTL_MS);
        if (outcome === "book") {
          this.pendingBookingConfirm.delete(row.handle);
          void this.bookPendingCalendar(row.handle, pb).catch((err) =>
            this.logger.error({ err }, "bookPendingCalendar threw"));
          return;
        }
        if (outcome === "skip") {
          this.pendingBookingConfirm.delete(row.handle);
          void this.send(row.handle, "👍 skipped").catch(() => {});
          return;
        }
        if (outcome === "expired") this.pendingBookingConfirm.delete(row.handle);
    ...
    if (!isGroup && row.handle && !this.killSwitch && text && !this.isOwnerChatRow(row)) {
      const relForAgent = this.ownerProfileStore.relationshipFor(row.handle, this.contactNames.get(row.handle));
      if (IMessageSession.SPOUSE_AGENT_ENABLED && isInnerCircle(relForAgent)) {
        // Spouse-nudge-agent: if the message is an action-item list, plan +
        // EXECUTE (todos + calendar), confirm to her, and heads-up the owner —
        // then skip the normal reply (the confirmation IS the reply).
        if (await this.runSpouseAgent(row.handle, text, this.contactNames.get(row.handle))) {
          return;
        }
        // no action items → fall through to the normal reply below
      } else if (IMessageSession.CONCIERGE_ENABLED) {
        // VIP task capture (single-item) — fire-and-forget, never delays reply.
    ...
    if (!tellCheck.ok) {
      ...
      if (!(retry && retryCheck.ok) && !isGroup) {
        try {
          const exemplars = ownerVoiceExemplars(this.ownerVoiceGlobal.map((t) => ({ text: t })), { max: 3, relevantTo: text });
          third = await this.agent.respondTo(row.handle, userText, systemHint + "\n\n" + strictRegenHint({ ownerName, reasons: [tellCheck.reason ?? "", retryCheck.reason ?? ""], exemplars }), { turnHint: replyTier, timeoutMs: 20_000 });
        } catch (err) {
          this.logger.warn({ err, from: row.handle }, "strict third regeneration threw");
        }
        if (third && isNoReplySentinel(third)) {
          this.logger.info({ from: row.handle }, "strict third regeneration abstained ([[NO_REPLY]]) — staying silent");
          return;
        }
      ...
      if (retry && retryCheck.ok) {
        ...
        this.logger.info({ from: row.handle, draftPreview: third.slice(0, 120) }, "strict third regeneration accepted — continuing into the normal gates");
        draft = third;
        ...
        this.logger.warn({ from: row.handle, reason: thirdCheck.reason ?? retryCheck.reason, draftPreview: (third ?? retry ?? draft).slice(0, 120) }, "NO reply — draft suppressed by bot-tell filter three times; falling back to greeting/owner heads-up");
        this.broadcast({
          type: "activity",
          data: {
            kind: "agent_skipped",
  ...
  // it on its native wire. Mirrors the cross-bridge TEXT hop in deliverPendingSend.
  // The file lives on the shared Mac, so we send the ABSOLUTE PATH (not bytes) —
  // the peer re-checks it's inside the personal-docs roots before attaching.
  private async relayDocToPeer(
    ...
    const headers: Record<string, string> = { "Content-Type": "application/json" };
    // Authenticate to the PEER's token: this helper only ever dials the WhatsApp
    // bridge, whose /session routes validate LANTERN_BRIDGE_TOKEN (NOT our own
    // LANTERN_IMESSAGE_BRIDGE_TOKEN). Matches the existing cross-bridge text hop.
    const tok = process.env.LANTERN_BRIDGE_TOKEN;
    if (tok) headers["Authorization"] = `Bearer ${tok}`;
    const ctrl = new AbortController();
  ...
  // Cross-bridge document relay (receive side): the PEER bridge (WhatsApp) calls
  // this via POST /session/:tenantId/send-doc when the owner asked it to send a
  // doc over iMessage/SMS. SECURITY: re-validate the path is inside our own
  ...
  async receiveRelayedDoc(to: string, filePath: string, caption?: string): Promise<{ ok: boolean; reason?: string }> {
    ...
    return this.deliverFileToContact(to, filePath, caption || "file");
  }
  ...
  // resolution live in ONE place (no drift between the two entry points).
  // Always routes through relayDocRequestToOwner, which confirms with the owner
  // before any file leaves — never auto-sends a sensitive document.
  private async dispatchOwnerDocSend(
    jid: string,
    d: { channel: string; contact: string; doc: string },
  ): Promise<void> {
    try {
      ...
      // Honor the channel the owner named. "whatsapp" is cross-bridge: confirm
      // here, then relay the confirmed file to the WhatsApp bridge on send
      // (relayDocToPeer). "imessage"/"sms"/"auto" deliver on this bridge's wire.
      const targetChannel = d.channel === "whatsapp" ? "whatsapp" : d.channel === "sms" ? "sms" : "imessage";
      await this.relayDocRequestToOwner(resolved.phone, resolved.name || d.contact, d.doc, true, targetChannel);
    } catch (err) {
  ...
  private async handleOwnerDocQuery(
    ...
    const systemHint = [
      ...
      "3. Email HISTORY (travel history, past bookings/receipts, USCIS/immigration notices, old bills) →",
      "   a. search_email — the LOCAL index of the entire mailbox (no login, always works). Run several 1-3 keyword searches (airline names, 'itinerary', 'booking confirmed', 'uscis'), not one long query. Each result carries a rowid.",
      "   b. read_email(rowid=…) — reads that message's BODY from the local mailbox. When the subject alone doesn't answer it (appointment/confirmation TIMES, addresses, order details, amounts, codes), take the rowid from a search_email hit and read the body. NEVER say you can't read the email — you can.",
      "   c. gmail_search only ADDS live detail; if it errors, search_email + read_email stand on their own.",
      "",
      ...
      "  • Don't restate the thing the owner is already looking at (don't echo the full code/message back at them) — they can see it; just tell them what it IS.",
      "",
      "# Follow-up offers — only when they genuinely help (NOT a checklist)",
      "  • Reason about whether a follow-up is actually useful before adding one. A helpful offer is fine; a reflexive one is noise. When in doubt, don't — just answer.",
      ...
      "  • A durable ID the owner will need later (passport #, policy #, account #) MAY warrant a one-line 'want this as a note?' — but at most ONE offer, and only if it reads as genuinely useful, never tacked onto every answer.",
      ...
      "SENDDOC: when the owner asks you to send a FILE / document / copy / scan / their ID (PAN card, aadhaar, passport, licence, receipt, insurance, certificate, 'send Manasa my pan card', 'forward the passport scan to dad') you MUST emit [SENDDOC:...] — NOT [SEND]. [SEND] carries only text and CANNOT attach a file, so using it for a document sends a 'here's the pan card' message with no file attached (a lie). [SENDDOC] finds the owner's real file, shows the owner the filename, and attaches it after they reply 'send'. Put the document name (e.g. 'PAN card') in the third field, not a sentence. The FIRST field is the CHANNEL and you MUST honor what the owner said: 'imessage' when they say iMessage/message, 'whatsapp' for WhatsApp, 'sms' for SMS/text, else 'auto' when unstated — e.g. 'send Chikka my passport via iMessage' → [SENDDOC:imessage|Chikka|passport]. Never leave it 'auto' when they named a channel. If they ask for both a note AND a file, emit BOTH a [SEND] and a [SENDDOC].",
    ...
    for (const ev of calendarEvents) {
      ...
      // when the calendar wasn't loaded for this turn the list is empty and this
      // is a no-op. Tell the owner instead of silently booking a conflict.
      ...
      if (conflict.conflict) {
        ...
        await this.send(jid, `heads up — that overlaps your "${conflict.title}" at ${when}; want a different time?`);
        continue;
      }
      try {
    ...
    for (const d of sendDocs) {
      await this.dispatchOwnerDocSend(jid, d);
    }
    // BACKSTOP (marker-drop safety — the exact production incident): if the
    // persona answered "yeah, sent your passport to Chikka" conversationally and
    // dropped the [SENDDOC] marker, the send silently never fired. Mirror the
    // inbound doc-intent classifier: when the owner's text reads as a doc send
    // but no marker was emitted, classify it and drive the SAME confirm→deliver
    // relay. Fail-safe by construction (parseDocIntent → no send on any doubt);
    // the owner still confirms every relay before a file leaves.
    if (sendDocs.length === 0 && maybeDocRequest(query)) {
      try {
        ...
        if (intent.isDocRequest && intent.document && intent.contact) {
          ...
          await this.dispatchOwnerDocSend(jid, {
