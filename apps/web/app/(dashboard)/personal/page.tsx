        ...
        <Panel
          ...
          action={
            commitments.length > 0 ? (
              <span className="inline-flex h-5 items-center justify-center rounded-full bg-amber-500/15 px-2 text-[12px] font-medium text-amber-300">
                {commitments.length}
              </span>
            ) : undefined
          }
  ...
  return (
    <li className="py-3">
      <div className="flex items-baseline justify-between gap-3">
        ...
        <time className="font-mono text-[11px] uppercase tracking-wide text-zinc-600">
      ...
      {LIFE_DOMAINS.map((domain) => {
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between">
              <Icon className="h-4 w-4 text-zinc-500 group-hover:text-zinc-300" />
              <span className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wide text-zinc-600">
                <span className={`h-1.5 w-1.5 rounded-full ${statusDot}`} />
                {statusLabel}
              </span>
            </div>
            <div className="mt-2 text-sm font-medium text-zinc-200">{domain.label}</div>
            <div className="mt-0.5 text-[12px] text-zinc-500">{domain.hint}</div>
            {(openCount > 0 || recordCount > 0) && (
              <div className="mt-2 flex gap-2">
                {openCount > 0 && (
                  <span className="inline-flex items-center rounded-full bg-amber-500/10 px-1.5 py-0.5 text-[11px] font-medium text-amber-300">
                    {openCount} open
                  </span>
                )}
                {recordCount > 0 && (
                  <span className="inline-flex items-center rounded-full bg-zinc-700/40 px-1.5 py-0.5 text-[11px] font-medium text-zinc-400">
                    {recordCount} records
                  </span>
                )}
              </div>
