...
const defaultGeneral: GeneralSettings = {
  ...
  workspaceId: "t_acme_01hq3x9k7m",
  defaultModel: "auto",
  defaultIsolation: "standard",
};

const defaultProviders: ProviderSettings = {
  openai: { key: "", status: "not_configured" },
  anthropic: { key: "", status: "not_configured" },
...
export default function SettingsPage() {
  ...
  const handleCopyWorkspaceId = async () => {
    await navigator.clipboard.writeText(general.workspaceId);
  ...
  const updateProvider = useCallback(
    (provider: keyof ProviderSettings, key: string) => {
      setProviders((prev) => ({
        ...prev,
        [provider]: { ...prev[provider], key, status: key ? prev[provider].status : "not_configured" as ProviderStatus },
      }));
    },
    []
  );
  ...
  const handleInviteMember = (invited: InvitedMember) => {
    const stored: StoredMember = {
      ...
      email: invited.email,
      role: invited.role,
      joinedAt: invited.joinedAt.toISOString(),
    };
  ...
  const handleChangeRole = (memberId: string, newRole: string) => {
    const next = members.map((m) =>
      m.id === memberId ? { ...m, role: newRole as StoredMember["role"] } : m
    );
    setMembers(next);
    saveLS("members", next);
    ...
    <div className="flex flex-1 flex-col overflow-auto">
      ...
      <div className="border-b border-zinc-800 bg-surface-1 px-8 py-5">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Manage your workspace, API keys, providers, team, and billing.
        </p>
      </div>
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={clsx(
                ...
                activeTab === tab.id
                  ? "border-lantern-500 text-lantern-400"
      ...
      <div className="flex-1 p-8">
        {activeTab === "general" && (
          <GeneralTab
            ...
            onCopyId={handleCopyWorkspaceId}
          />
        )}
        ...
        {activeTab === "billing" && (
          <BillingTab
            ...
            onToggleHardLimit={handleToggleHardLimit}
          />
        )}
      </div>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to remove ${removeTarget?.name} (${removeTarget?.email}) from the team? They will lose access immediately.`}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                readOnly
                className="flex-1 rounded-lg border border-zinc-700 bg-surface-2 px-3 py-2 text-sm text-zinc-500 outline-none cursor-default font-mono"
              />
              <button
                onClick={onCopyId}
                className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-700 px-3 py-2 text-sm text-zinc-300 transition-colors hover:bg-surface-3"
              >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-600">
              The model router will use this preference unless overridden per-agent.
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-600">
              Untrusted code always runs in a microVM regardless of this setting.
            </p>
          </div>
        </div>
      </div>
...
function ApiKeysTab({
  ...
  onCreateClick: () => void;
  onCopyPrefix: (key: StoredApiKey) => void;
  onRevokeClick: (key: StoredApiKey) => void;
}) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-zinc-500">
            Manage keys used to authenticate with the Lantern API.
          </p>
        </div>
        <button
          onClick={onCreateClick}
          className="inline-flex items-center gap-2 rounded-lg bg-lantern-500 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-lantern-400"
        >
          Create API Key
        </button>
      </div>
              ...
              <tr key={key.id}>
                ...
                <td>
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded px-2 py-1 text-xs text-red-400 transition-colors hover:bg-red-500/10"
                  >
          ...
          <div className="overflow-hidden rounded-xl border border-zinc-800 bg-surface-1 opacity-60">
            <table className="data-table">
              <tbody>
                {revokedKeys.map((key) => (
                  <tr key={key.id}>
                    <td className="font-medium text-zinc-500">{key.name}</td>
                    <td>
                      <code className="rounded bg-surface-3 px-2 py-0.5 font-mono text-xs text-zinc-600 line-through">
                        {key.prefix}...
                      </code>
                    </td>
...
function ProvidersTab({
  ...
  saving: boolean;
  onKeyChange: (provider: keyof ProviderSettings, key: string) => void;
  onTest: (provider: keyof ProviderSettings) => Promise<boolean>;
  onSave: () => void;
}) {
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="mt-0.5 text-xs text-zinc-500">
          Configure API keys for the LLM providers your agents will use. The model router maps capability requests to these providers.
        </p>
      </div>

      <ProviderCard
        name="OpenAI"
        description="GPT-4o, GPT-4, o3, and more"
        icon={<span className="text-sm font-bold text-emerald-400">AI</span>}
        apiKey={providers.openai.key}
        status={providers.openai.status}
        onApiKeyChange={(k) => onKeyChange("openai", k)}
      ...
      <ProviderCard
        ...
        icon={<span className="text-sm font-bold text-orange-400">A</span>}
        apiKey={providers.anthropic.key}
        status={providers.anthropic.status}
        onApiKeyChange={(k) => onKeyChange("anthropic", k)}
      ...
      <ProviderCard
        ...
        icon={<span className="text-sm font-bold text-blue-400">G</span>}
        apiKey={providers.google.key}
        status={providers.google.status}
        onApiKeyChange={(k) => onKeyChange("google", k)}
        onTest={() => onTest("google")}
      />

      <div className="rounded-lg border border-zinc-800 bg-surface-2 px-4 py-3">
        <p className="text-xs text-zinc-500">
          Keys are encrypted at rest and never leave your infrastructure. They are only decrypted inside the microVM runtime at execution time.
        </p>
      </div>
...
function TeamTab({
  ...
  onRemoveClick: (member: StoredMember) => void;
}) {
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-zinc-700 px-3 py-1.5 text-sm font-medium text-zinc-300 transition-colors hover:bg-surface-3"
        >
          Invite Member
        </button>
      </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={clsx(
                ...
                plan.current
                  ? "border-lantern-500 bg-lantern-500/5"
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-lg font-bold text-lantern-400">{plan.price}</p>
                </div>
                {plan.current && (
                  <span className="rounded-full bg-lantern-500/10 px-2 py-0.5 text-xs font-medium text-lantern-400">
                    Current
                  </span>
                )}
              </div>
              ...
              {!plan.current && (
                <button className="mt-4 inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-zinc-700 px-3 py-1.5 text-xs font-medium text-zinc-300 transition-colors hover:bg-surface-3">
                  ...
                  <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-surface-3">
                    <div
                      className={clsx(
                        ...
                        pct > 90 ? "bg-red-500" : pct > 70 ? "bg-amber-500" : "bg-lantern-500"
                      )}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
        ...
        <div className="overflow-hidden rounded-xl border border-zinc-800 bg-surface-1">
          <table className="data-table">
            ...
            <tbody>
              {costByAgent.map((row) => (
                <tr key={row.agent}>
                  <td className="font-medium text-zinc-300">{row.agent}</td>
                  <td className="text-zinc-400">{row.runs}</td>
                  <td className="text-zinc-400">{row.tokens}</td>
                  <td className="font-medium text-zinc-200">{row.cost}</td>
                </tr>
              ))}
              <tr className="border-t border-zinc-700">
                <td className="font-semibold text-zinc-100">Total</td>
                <td className="text-zinc-400">348</td>
                <td className="text-zinc-400">2.1M</td>
                <td className="font-semibold text-lantern-400">$12.47</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-surface-1 p-6 space-y-5">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  onChange={(e) => setBilling({ ...billing, budgetLimit: e.target.value })}
              ...
              <button
                onClick={onSetBudget}
                className="rounded-lg border border-zinc-700 px-4 py-2 text-sm font-medium text-zinc-300 transition-colors hover:bg-surface-3"
              >
                Set Budget
              </button>
            </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-zinc-800 bg-surface-2 px-4 py-3">
            <div>
              <p className="text-sm font-medium text-zinc-200">Hard limit</p>
              <p className="text-xs text-zinc-500">
                Pause all agents when budget is exceeded. If disabled, you will only receive a warning.
              </p>
            </div>
            <button
              onClick={onToggleHardLimit}
              className="text-zinc-400 transition-colors hover:text-zinc-200"
            >
              {billing.hardLimit ? (
                <ToggleRight className="h-8 w-8 text-lantern-500" />
              ) : (
