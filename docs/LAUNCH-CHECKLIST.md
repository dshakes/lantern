...
- [x] **Firecracker live cold-boot validated** — `infra/firecracker/integration-test.sh` boots a REAL microVM end-to-end on KVM (backend available + microVM booted both PASS), verified on an Apple Silicon Lima nested-virt guest (`infra/lima/`). Fixed six bugs the test had never exercised (rustls crypto provider, firecracker `--log-level`, unbuilt `certs.img`, missing vsock dir, swallowed error chain, restore mem-path). `.github/workflows/microvm-integration.yml` runs it on the KVM runner. REMAINING: the in-guest harness VendSecret agent (below) and CI runner provisioning
- [ ] **In-guest harness VendSecret agent** — the microVM boots with the harness as PID 1, but `services/harness/manager_client.rs` is a stub: it does not yet read vm_id + cert paths from `/proc/cmdline`, mount the read-only `certs` drive, build the mTLS client, and call `VendSecret`. The manager side (per-VM cert issuance, certs.img, fail-closed) is done; this is the guest half. Gated in the test behind `EXPECT_VENDSECRET=1`
